    ...
    <section>
      <Wrapper>
        ...
        <div>
          ...
          <p className='text-xl mt-6 text-slate-600 max-w-screen-sm'>After completing the first three quarters, the participants will select one or more specializations consisting of two courses each:</p>
        </div>
        <div className='mt-10 flex flex-col-reverse lg:flex-row gap-x-6 gap-y-8'>
          {/* Content Left */}
          <div className='shadow-xl rounded-xl border border-slate-200 py-8 px-8 w-8/12 self-start sticky top-28 mt-3 ml-10'>
            <h4 className=' text-pr text-lg mt-3 font-medium'>Specialized Program</h4>
            <h3 className='text-2xl font-extrabold mt-4'>{selectedItemData?.header}</h3>
            <p className='text-lg text-slate-600 mt-4'>{selectedItemData?.description}</p>
            <button className='text-primary text-xl mt-4 underline flex items-end gap-x-2'>Learn More
          ...
          <div className='flex-1 space-y-4 ml-4 px-4 py-6 w-4/12'>
            <h4 className='text-primary font-semibold'>Specialized programs</h4>
            {ProgramData.map((item) => (
              <div key={item.slug}>
                <div onClick={() => setSelectedItem(item.slug)} className='flex gap-x-4 items-center cursor-pointer'>
